...
# Read the CSV file
df = pd.read_csv('EOL survey analysis v2.csv')

# Clean column names
df.columns = df.columns.str.strip()

# Exclude Respondent # column
...
def create_experience_analysis():
    ...
    create_plot(faith_means, 'Mean Responses by Faith Group', 'faith_group_analysis.png')
...
# 3. Comfort Level Analysis
def create_comfort_analysis():
    # Get response columns
    response_cols = get_response_columns(df)
    ...
    create_plot(comfort1, 'Mean Responses by Comfort Level (Scenario 1)', 'comfort_level_scenario1_analysis.png')
    
    # Comfort Level 2
    comfort2 = df.groupby('S2 Q5. If not, do you feel comfortable providing end-of-life care, such as offering prayer outside your faith or performing last rites?')[response_cols].mean()
    create_plot(comfort2, 'Mean Responses by Comfort Level (Scenario 2)', 'comfort_level_scenario2_analysis.png')
